...
//     .split("-")
//     .splice(-1)[0];
//   const path_name = getPathName;
//   const select_catgery = Object.keys(SubDonationCatgery).filter((data) =>
//     data.toLocaleLowerCase().includes(path_name)
//   );

//   pathName.split("/").splice(-1)[0].split("-").join(" ").toLowerCase();
//   const [catgery, setCatgery] = useState(String(select_catgery));
...
//         <div className="w-[90%] sticky top-[80px]  no_padding_margin ">
//           <H2
//             text={catgery}
//             className="xsm:text-center lg:text-start w-full py-14 bg-white z-index"
//           />
//         </div>
//         <div className="w-full h-[1px] bg-[#EDEFF1] no_padding_margin"></div>
//         <div className="w-[90%]">
...
//                 <div className="lg:w-[30%] xsm:w-full   relative -z-20">
//                   <DropDown
//                     active={active}
//                     setActive={setActive}
//                     list={CurrencyType}
...
const Index: FC = () => {
  const pathName = usePathname();
  // const value = pathName.split("/").splice(-1)[0];
  // console.log(value);
  const data = pathName.split("/").splice(-1)[0].split("-").splice(-1)[0];
  return <RazerPayFrom value={data} />;
};
